...
BIRD SCOREで確認した試合の開始時刻から終了時刻までをMP4として保存し、開始前・終了後にそれぞれ60秒のバッファを付けます。終了時刻を確認できない試合は、推測で切り出さず対象外として警告します。既存の出力動画は実測長を照合し、長さが一致しない旧生成物は`.old-duration.mp4`として退避してから再生成します。元アーカイブと旧形式動画は削除しません。ファイル名・ディレクトリ名には空白を入れず、中断後も続きから実行できます。
