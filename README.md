...
BIRD SCOREで確認した試合の開始時刻から終了時刻までをMP4として保存し、開始前・終了後にそれぞれ60秒のバッファを付けます。終了時刻を確認できない試合は、推測で切り出さず対象外として警告します。既存の出力動画は実測長を照合し、長さが一致しない派生動画は正しい範囲で再生成します。元アーカイブは削除しません。ファイル名・ディレクトリ名には空白を入れず、中断後も続きから実行できます。
